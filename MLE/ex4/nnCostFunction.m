...
% backpropagation starts here
   error = h - y(i, :);
   gradz2 = sigmoidGradient(z2);
   delta2 = zeros(size(Theta2,2),1);
   gradz2 = [ones(1,1); gradz2];
   delta2 = Theta2'*error'.*gradz2;
    
    delta2 = delta2(2:end,1);
   
   for j=1:size(delta2,1)
       for k= 1:size(a1,1)
           Theta1_grad(j,k) = Theta1_grad(j,k) +  delta2(j)*a1(k);
       end
   end
   for j=1:size(error,2)
       for k= 1:size(a2,1)
           Theta2_grad(j,k) = Theta2_grad(j,k) +  error(j)*a2(k);
       end
   end
